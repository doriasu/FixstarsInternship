...
char* i2c_read(int fd, uint32_t addr, const void *yomikomi_reg,
                    size_t len);
int main(void) {
  //設定値
  uint8_t settei[][2] = {{0xff, 0x00},
                         {0x2c, 0xff},
                         {0x2e, 0xdf},
                         {0xff, 0x01},
                         {0x3c, 0x32},
  ...
  for (int i = 0; i < (int)sizeof(gazou) / 2; i++) {
  ...
  if (spi_cmdread(fd_spi, 0, burst, sizeof(burst), gaso, sizeof(gaso)) < 0) {
    perror("読み込みに失敗しました\n");
    return 0;
  }
  //ファイル書き出し
  ...
  uint8_t size_gazo = sizeof(gaso) - 1;
